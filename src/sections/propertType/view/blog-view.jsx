...
export default function PropertyType() {
  ...
  const getUsersData = async () => {
    ...
    if (!selectedResidential) {
      ...
      try {
      const response = await axios.get(`${baseUrl}post-property/get-residentials-items?id=${selectedResidential}`, {
        headers: {
          Authorization: token,
        },
      });
  ...
  const getResidentialData = async () => {
    ...
    try {
      const response = await axios.get(`${baseUrl}post-property/get-residential`, {
        headers: {
          Authorization: token,
        },
      });
      console.log("resposne.data",response)
      setResidentialData(response?.data?.data?.amenities || []);
    } catch (error) {
  ...
  const handleFormSubmit = async (newData) => {
    try {
      ...
      await axios.post(
        `${baseUrl}post-property/create-residential-items`,
        { nameEN: newData.itemEnglish, nameHN: newData.itemHindi,residentialId:selectedResidential  },
        {
          headers: {
            Authorization: token,
          },
