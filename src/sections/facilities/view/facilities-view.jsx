...
export default function FacilitiesData() {
  ...
  const getUsersData = async () => {
    ...
    try {
      const response = await axios.get(`${baseUrl}post-property/get-facilities-data`, {
        headers: {
          Authorization: token,
        },
      });
  ...
  const handleFormSubmit = async (newData) => {
    try {
      ...
      await axios.post(
        `${baseUrl}post-property/create-facilities`,
        ...
        <Scrollbar>
          <TableContainer sx={{ overflow: 'unset' }}>
            <Table sx={{ minWidth: 800 }}>
              ...
              />
              <TableBody>
                {postData.length>0 ? (postData.map((row) => (
                  <PostTableRow key={row._id} nameEN={row.nameEN} nameHN={row.nameHN} id={row._id} onModification={getUsersData} />
                ))):(<TableRow>
