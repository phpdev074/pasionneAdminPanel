...
export default function RoomFacilitiesData() {
  ...
  const getUsersData = async () => {
    ...
    try {
      const response = await axios.get(`${baseUrl}post-property/get-Room-Facilities-data`, {
        headers: {
          Authorization: token,
        },
      });
  ...
  const handleFormSubmit = async (newData) => {
    try {
      ...
      await axios.post(
        `${baseUrl}post-property/create-room-facilities`,
      ...
      getUsersData();
      setSnackbarMessage('Room Facilities data added successfully');
  ...
  return (
    <Container>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
        <Typography variant="h4">Room Facility</Typography>
