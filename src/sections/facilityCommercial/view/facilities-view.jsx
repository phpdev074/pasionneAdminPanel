...
export default function FacilityCommercial() {
  ...
  const getUsersData = async () => {
    ...
    if (!selectedResidential) {
      ...
      try {
      const response = await axios.get(`${baseUrl}post-property/get-fac-comm-item?id=${selectedResidential}`, {
        headers: {
          Authorization: token,
        },
      });
  ...
  const getResidentialData = async () => {
    ...
    try {
      const response = await axios.get(`${baseUrl}post-property/get-fac-commercial`, {
        headers: {
          Authorization: token,
        },
      });
  ...
  const handleFormSubmit = async (newData) => {
    try {
      ...
      await axios.post(
        `${baseUrl}post-property/create-fac-comm-item`,
        { nameEN: newData.itemEnglish, nameHN: newData.itemHindi,commercialId:selectedResidential  },
        {
          headers: {
            Authorization: token,
          },
  ...
  return (
    <Container>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
        <Typography variant="h4">Add Facility Commercial</Typography>
